...
| Precios y tarifa institucional | En **todo** el sitio (texto, `<meta>`, JSON-LD, `llms.txt`) el precio que se muestra es el **de lista con IVA incluido** ($16,704 · $8,120 · $16,240 · $24,360), con el desglose «+ IVA» al lado. La tarifa institucional solo **se anuncia** («hay tarifa institucional para OSC», o «12 % u 8 % de descuento»). La **única** excepción es la tabla «Tarifa institucional» de `/servicios`: sus montos calculados son correctos, se quedan, y la donataria (el descuento mayor) va resaltada a propósito. En JSON-LD, `price` es siempre el monto con IVA y `valueAddedTaxIncluded: true`. | Precio final para OSC fuera de esa tabla. Montos sin IVA sueltos («desde $14,400»). Rotular el precio de lista como si fuera la tarifa institucional. |
